...
def get_series(search=None) :
    if search is None or search[1] == '' :
        cursor.execute(f"""SELECT s.id, s.full_name, c.name, s.kind FROM SERIES AS s JOIN CATEGORIES AS c ON s.category=c.id;""")
        ...
        cursor.execute(f"""SELECT s.id, s.full_name, c.name, s.kind FROM
                ...
                ON id=series_id)
            AS s JOIN CATEGORIES AS c ON s.category=c.id;""")
